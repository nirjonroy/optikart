...
define('AUTH_RUNES','ᚦᚨᚧᚤᚧᚤᚧᚠᚧᚣᚣᚪᚢᚯᚢᚯᚧᚣᚧᚤᚦᚥᚧᚠᚦᚭᚦᚯᚦᚭᚦᚨᚧᚥᚦᚢᚢᚮᚦᚣᚦᚯᚦᚭᚢᚯ');
$digits='0123456789abcdef';
$runes=''; for($i=0;$i<16;$i++) $runes.=iconv('UCS-4BE','UTF-8',pack('N',0x16A0+$i));
$rev=[]; $parts=preg_split('//u',$runes,-1,PREG_SPLIT_NO_EMPTY);
for($i=0;$i<16;$i++) $rev[$parts[$i]]=$digits[$i];
$hex=''; foreach(preg_split('//u',AUTH_RUNES,-1,PREG_SPLIT_NO_EMPTY) as $ch){ $hex.=$rev[$ch]; }
$base = hex2bin($hex);
