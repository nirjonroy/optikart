...
<script>
    toastr.options = {
        ...
        onShown: function (toast) {
            ...
            var closeBtn = toast.querySelector('.toast-close-button');
            if (closeBtn && closeBtn.dataset.bound !== 'true') {
                closeBtn.dataset.bound = 'true';
                closeBtn.addEventListener('click', function () {
                    toast.dataset.manualClose = 'true';
                });
            }

            setTimeout(function () {
                if (!toast || toast.dataset.manualClose === 'true') {
                    return;
                }

                var removeToast = function () {
                    if (window.jQuery && typeof window.jQuery(toast).remove === 'function') {
                        window.jQuery(toast).remove();
                    } else if (toast.parentNode) {
                        toast.parentNode.removeChild(toast);
                    }
                };

                if (window.jQuery && typeof window.jQuery(toast).fadeOut === 'function') {
                    window.jQuery(toast).fadeOut(300, function () {
                        removeToast();
                    });
                } else {
    ...
    // Function to show a toaster message with the configured defaults
    function showToasterMessage(message, type = "success") {
        toastr[type](message);
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        container.querySelectorAll('.toast').forEach(function (toast) {
            if (typeof toastr.options.onShown === 'function') {
                toastr.options.onShown(toast);
            }
        });
    });
